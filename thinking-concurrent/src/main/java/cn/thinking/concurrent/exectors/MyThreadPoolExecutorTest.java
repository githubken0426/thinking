...
//		test.testThreadPoolExecutor2(hanlder);
	...
	public void testThreadPoolExecutor2(RejectedExecutionHandler handler) {
		ThreadPoolExecutor threadPool = new ThreadPoolExecutor(1, 2, 3, TimeUnit.SECONDS,
	...
	class ThreadPoolTask implements Runnable, Serializable {
		...
		private Object threadPoolTaskData;// 保存任务所需要的数据
		...
		public void run() {
			System.out.println(Thread.currentThread() + "start " + threadPoolTaskData);// 处理一个任务
			try {
				TimeUnit.SECONDS.sleep(2);// 便于观察，等待一段时间
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public void testThreadPoolExecutor3(RejectedExecutionHandler handler) {
		 ...
		 */
		ThreadPoolExecutor threadPool = new ThreadPoolExecutor(1, 2, 3, TimeUnit.SECONDS,
				new ArrayBlockingQueue<Runnable>(queueDeep), handler);
		for (int i = 0; i < 10; i++) {
			try {
				TimeUnit.SECONDS.sleep(3);
			} catch (InterruptedException e1) {
