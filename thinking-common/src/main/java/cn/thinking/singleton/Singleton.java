...
public class Singleton {
    /*
             * 一、线程不安全. 这种写法lazy loading很明显，但是致命的是在多线程不能正常工作。
     */
    ...
    /*
             * 二、线程安全. 这种写法能够在多线程中很好的工作， 它也具备很好的lazy loading，
             *   但是，效率很低 大多数情况下不需要同步
     */
    ...
    /*
             *三、 这种方式基于classloder机制避免了多线程的同步问题， 不过，instance在类装载时就实例化，
    ...
    /*
             * 四、 这种方式同样利用了classloder的机制来保证初始化instance时只有一个线程
     */
    ...
    /*
             * 五、枚举单例 这种方式是Effective Java作者Josh Bloch 提倡的方式，
    ...
    /*
           ...
           * 问题：
     * 6.1， 如果单例由不同的类装载器装入，那便有可能存在多个单例类的实例。假定不是远端存取，
             *      例如一些servlet容器对每个servlet使用完全不同的类装载器，
             *      这样的话如果有两个servlet访问一个单例类，它们就都会有各自的实例。
     * 6.2， 如果Singleton实现了java.io.Serializable接口，那么这个类的实例就可能被序列化和复原。
